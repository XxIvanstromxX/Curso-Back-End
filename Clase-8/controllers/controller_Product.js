...
const buscarProductoPorId = async (req, res) => {
  const { id } = req.params; // Extrae el ID del producto de los parámetros de la solicitud
  ...
  try {
    const producto = await Producto.findById(id); // Busca el producto por ID
...
module.exports = {
  ...
  actualizarProducto,
  eliminarProducto,
  buscarProductoPorId, // Exporta la función para buscar un producto por ID
};
